...
FILE_PREFIX="AllArcSineTestFixture";
...
diff -b ${FILE_PREFIX}.bin ${CORDIC_DIR}/cordic-fixed-arcsine-table.bin;
if [ $? -eq 0 ]; then
	echo "[PASSED] All arcsine values match verified values";
else
	echo "[FAILED] Differences found in calculated versus verified arcsine values";
fi;
