...
FILE_PREFIX="AllArcCosineTestFixture";
...
diff -b ${FILE_PREFIX}.bin ${CORDIC_DIR}/cordic-fixed-arccosine-table.bin;
if [ $? -eq 0 ]; then
	echo "[PASSED] All arccosine values match verified values";
else
	echo "[FAILED] Differences found in calculated versus verified arccosine values";
fi;
